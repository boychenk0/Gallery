    ...
    function b(a, h) {
        ...
        this.connection = new b.ConnectionManager(this.key, b.Util.extend({getStrategy: function (a) {
            ...
            return new b.Timeline(e.key,
                e.sessionID, {features: b.Util.getClientFeatures(), params: e.config.timelineParams || {}, limit: 50, level: b.Timeline.INFO, version: b.VERSION})
        }, getTimelineSender: function (a, d) {
            return e.config.disableStats ? null : new b.TimelineSender(a, {encrypted: e.isEncrypted() || !!d.encrypted, host: e.config.statsHost, path: "/timeline"})
        }, activityTimeout: this.config.activity_timeout, pongTimeout: this.config.pong_timeout, unavailableTimeout: this.config.unavailable_timeout}, this.config, {encrypted: this.isEncrypted()}));
    ...
    a.subscribe = function (a) {
        ...
        this.connection.state === "connected" && c.authorize(this.connection.socket_id, function (f, g) {
            f ? c.handleEvent("pusher:subscription_error",
                g) : b.send_event("pusher:subscribe", {channel: a, auth: g.auth, channel_data: g.channel_data})
        });
        return c
    };
...
(function () {
    ...
    Pusher.getDefaultStrategy = function (b) {
        return[
            ...
            [":def", "sockjs_options", {hostUnencrypted: b.httpHost + ":" + b.httpPort, hostEncrypted: b.httpHost + ":" + b.httpsPort}],
            [":def", "timeouts", {loop: !0, timeout: 15E3, timeoutLimit: 6E4}],
            [":def", "ws_manager", [":transport_manager", {lives: 2, minPingDelay: 1E4, maxPingDelay: b.activity_timeout}]],
            [":def_transport",
...
(function () {
    ...
    a.connect = function (a, b) {
        ...
        var i = Pusher.Util.now(), j = f.pop().connect(a, function k(c, e) {
            if (c) {
                ...
                var g = Pusher.Util.now() - i, p = e.transport.name, o = Pusher.Util.getLocalStorage();
                if (o)try {
                    o.pusherTransport = JSON.stringify({timestamp: Pusher.Util.now(), transport: p, latency: g})
                } catch (r) {
                }
                b(null, e)
            }
...
(function () {
    ...
    c.connect = function (a, b) {
        var c = this, e = this.strategies, f = 0, g = this.timeout, i = null, j = function (m, k) {
            k ? b(null, k) : (f += 1, c.loop && (f %= e.length), f < e.length ? (g && (g *= 2, c.timeoutLimit && (g = Math.min(g, c.timeoutLimit))), i = c.tryStrategy(e[f],
                a, {timeout: g, failFast: c.failFast}, j)) : b(!0))
        }, i = this.tryStrategy(e[f], a, {timeout: g, failFast: this.failFast}, j);
        return{abort: function () {
            i.abort()
        }, forceMinPriority: function (b) {
            a = b;
...
(function () {
    ...
    a.connect = function (a, b) {
        ...
        var e = this, f = !1, g = this.transport.createConnection(this.name, this.priority, this.options.key, this.options), i = null, j = function () {
...
(function () {
    function b(a, b, c, e) {
        ...
        this.options = {encrypted: Boolean(e.encrypted), hostUnencrypted: e.hostUnencrypted, hostEncrypted: e.hostEncrypted}
    }
...
(function () {
    ...
    c.createConnection = function (a, b, c, e) {
        var f = this.transport.createConnection(a, b, c, e), g = this, i = null, j = null, m = function () {
            f.unbind("open", m);
            i = Pusher.Util.now();
            ...
            if (a.code === 1002 || a.code ===
                1003)g.manager.reportDeath(); else if (!a.wasClean && i && (a = Pusher.Util.now() - i, a < 2 * g.maxPingDelay))g.manager.reportDeath(), g.pingDelay = Math.max(a / 2, g.minPingDelay)
        };
        f.bind("open", m);
        return f
    };
    ...
    c.bindListeners = function () {
        ...
        a.onMessage = function (b) {
            ...
            try {
                ...
                c.action === "connected" ? a.finish("connected", {connection: new Pusher.Connection(c.id, a.transport)}) : (a.finish(c.action, {error: c.error}), a.transport.close())
            } catch (e) {
                a.finish("error", {error: e}), a.transport.close()
            }
        };
        a.onClosed = function (b) {
            a.unbindListeners();
            var c = Pusher.Protocol.getCloseAction(b) || "backoff", b = Pusher.Protocol.getCloseError(b);
            a.finish(c, {error: b})
        };
...
(function () {
    ...
    c.connect = function () {
        ...
        if (!a.connection && a.state !== "connecting")if (a.strategy.isSupported())if (Pusher.Network.isOnline() === !1)a.updateState("unavailable");
            ...
            a.timelineSender = a.options.getTimelineSender(a.timeline, {encrypted: a.encrypted}, a);
            var b = function (c, e) {
                c ? a.runner = a.strategy.connect(0, b) : e.action === "error" ? a.timeline.error({handshakeError: e.error}) : (a.runner.abort(), a.handshakeCallbacks[e.action](e))
            };
    ...
    c.send_event = function (a, b, c) {
        return this.connection ? this.connection.send_event(a,
            b, c) : !1
    };
    ...
    c.resetActivityCheck = function () {
        ...
        if (!this.connection.supportsPing()) {
            var a = this;
            a.activityTimer =
                new Pusher.Timer(a.options.activityTimeout, function () {
    ...
    c.updateState = function (a, b) {
        var c = this.state;
        this.state = a;
        c !== a && (Pusher.debug("State changed", c + " -> " + a), this.timeline.info({state: a}), this.emit("state_change", {previous: c, current: a}), this.emit(a, b))
    };
...
(function () {
    ...
    Pusher.Channel.Authorizer.prototype = {composeQuery: function (b) {
        var b = "&socket_id=" + encodeURIComponent(b) + "&channel_name=" + encodeURIComponent(this.channel.name), a;
