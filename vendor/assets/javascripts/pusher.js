...
(function () {
    function b(a, h) {
        ...
        this.connection = new b.ConnectionManager(this.key, b.Util.extend({getStrategy: function (a) {
            return b.StrategyBuilder.build(b.getDefaultStrategy(), b.Util.extend({}, e.options, a))
        }, getTimeline: function () {
            return new b.Timeline(e.key, e.sessionID, {features: b.Util.getClientFeatures(), params: e.options.timelineParams ||
            {}, limit: 25, level: b.Timeline.INFO, version: b.VERSION})
        }, getTimelineSender: function (a, d) {
            return e.options.disableStats ? null : new b.TimelineSender(a, {encrypted: e.isEncrypted() || !!d.encrypted, host: b.stats_host, path: "/timeline"})
        }, activityTimeout: b.activity_timeout, pongTimeout: b.pong_timeout, unavailableTimeout: b.unavailable_timeout}, this.options, {encrypted: this.isEncrypted()}));
    ...
    b.warn = function () {
        window.console && window.console.warn ? window.console.warn(b.Util.stringify.apply(this, arguments)) : b.log && b.log(b.Util.stringify.apply(this, arguments))
    };
    b.ready = function () {
        b.isReady = !0;
    ...
    a.subscribe = function (a) {
        var b = this, c = this.channels.add(a, this);
        this.connection.state === "connected" && c.authorize(this.connection.socket_id, this.options, function (f, g) {
            f ? c.emit("pusher:subscription_error", g) : b.send_event("pusher:subscribe", {channel: a, auth: g.auth, channel_data: g.channel_data})
        });
        return c
    };
    ...
    a.isEncrypted = function () {
        return b.Util.getDocumentLocation().protocol === "https:" ? !0 : !!this.options.encrypted
    };
    this.Pusher = b
}).call(this);
(function () {
    Pusher.Util = {now: function () {
        ...
        arrayIndexOf: function (b, c) {
            ...
            return Pusher.Util.keys(Pusher.Util.filterObject({ws: Pusher.WSTransport,
                flash: Pusher.FlashTransport}, function (b) {
...
(function () {
    ...
    Pusher.getDefaultStrategy = function () {
        return[
            [":def", "ws_options", {hostUnencrypted: Pusher.host + ":" + Pusher.ws_port, hostEncrypted: Pusher.host + ":" + Pusher.wss_port}],
            [":def", "sockjs_options", {hostUnencrypted: Pusher.sockjs_host + ":" + Pusher.sockjs_http_port, hostEncrypted: Pusher.sockjs_host + ":" + Pusher.sockjs_https_port}],
            ...
            [":def", "strategy", [":cached", 18E5, [":first_connected", [":if", [":is_supported", ":ws"], [":best_connected_ever", ":ws_loop", [":delayed", 2E3, [":sockjs_loop"]]], [":if", [":is_supported",
...
(function () {
    ...
    Pusher.Errors = {UnsupportedTransport: b("UnsupportedTransport"), UnsupportedStrategy: b("UnsupportedStrategy"), TransportPriorityTooLow: b("TransportPriorityTooLow"), TransportClosed: b("TransportClosed")}
}).call(this);
(function () {
    function b(a) {
...
(function () {
    ...
    function a(a, b) {
        var d = Pusher.Util.getDocument(), g = d.getElementsByTagName("head")[0], d = d.createElement("script");
...
(function () {
    ...
    Pusher.Dependencies = new Pusher.DependencyLoader({cdn_http: Pusher.cdn_http, cdn_https: Pusher.cdn_https, version: Pusher.VERSION, suffix: Pusher.dependency_suffix});
...
(function () {
    ...
    Pusher.Base64 = {encode: function (b) {
...
(function () {
    ...
    b.send = function (a, b) {
        var c = new Pusher.JSONPRequest({url: a.url, receiver: a.receiverName, tagPrefix: a.tagPrefix}), f = a.receiver.register(function (a, d) {
            c.cleanup();
            b(a, d)
        });
    ...
    a.send = function (a, b, e) {
        ...
        var f = this.options.tagPrefix || "_pusher_jsonp_", b = Pusher.Util.extend({}, b, {receiver: this.options.receiver}), b = Pusher.Util.map(Pusher.Util.flatten(c(Pusher.Util.filterObject(b, function (a) {
            return a !== void 0
...
(function () {
    ...
    b.DEBUG = 7;
    c.log = function (a, b) {
        if (this.options.level === void 0 || a <= this.options.level)this.events.push(Pusher.Util.extend({}, b, {timestamp: Pusher.Util.now(), level: a})), this.options.limit && this.events.length > this.options.limit && this.events.shift()
    };
    c.error = function (a) {
        this.log(b.ERROR, a)
    };
    ...
    c.send = function (a, b) {
        var c = this, e = {};
        this.sent === 0 && (e = Pusher.Util.extend({key: this.key, features: this.options.features, version: this.options.version}, this.options.params || {}));
...
(function () {
    ...
    c.send = function (a) {
        if (!this.timeline.isEmpty()) {
            var b = this.options, c = "http" + (this.isEncrypted() ? "s" : "") + "://";
            this.timeline.send(function (a, f) {
                return Pusher.JSONPRequest.send({data: a, url: c + b.host + b.path, receiver: Pusher.JSONP}, f)
            }, a)
        }
    };
...
(function () {
    ...
    a.connect = function (a, b) {
        var e = c(), f = [this.strategy];
        if (e && e.timestamp + this.ttl >= Pusher.Util.now()) {
            var g = this.transports[e.transport];
            g && (this.timeline.info({cached: !0, transport: e.transport}), f.push(new Pusher.SequentialStrategy([g],
                {timeout: e.latency * 2, failFast: !0})))
        }
        var j = Pusher.Util.now(), k = f.pop().connect(a, function i(c, g) {
            if (c) {
                var e = Pusher.Util.getLocalStorage();
                e && e.pusherTransport && delete e.pusherTransport;
                f.length > 0 ? (j = Pusher.Util.now(), k = f.pop().connect(a, i)) : b(c)
            } else {
                var e = Pusher.Util.now() - j, o = g.transport.name, n = Pusher.Util.getLocalStorage();
                if (n)try {
                    n.pusherTransport = JSON.stringify({timestamp: Pusher.Util.now(), transport: o, latency: e})
                } catch (p) {
                }
                b(null, g)
            }
...
(function () {
    ...
    c.connect = function (a, b) {
        var c = this.strategy, e, f = new Pusher.Timer(this.options.delay, function () {
            e = c.connect(a, b)
        });
        return{abort: function () {
            f.ensureAborted();
...
(function () {
    ...
    c.connect = function (a, b) {
        var c = this, e = this.strategies, f = 0, g = this.timeout, j = null, k = function (m, i) {
            i ? b(null, i) : (f += 1, c.loop && (f %= e.length), f < e.length ? (g && (g *= 2, c.timeoutLimit && (g = Math.min(g, c.timeoutLimit))), j = c.tryStrategy(e[f],
                a, {timeout: g, failFast: c.failFast}, k)) : b(!0))
        }, j = this.tryStrategy(e[f], a, {timeout: g, failFast: this.failFast}, k);
        return{abort: function () {
            j.abort()
        }, forceMinPriority: function (b) {
            a = b;
    ...
    c.tryStrategy = function (a, b, c, e) {
        var f = null, g = null, g = a.connect(b, function (a, b) {
...
(function () {
    ...
    var a = b.prototype;
    a.isSupported = function () {
        return this.transport.isSupported({disableFlash: !!this.options.disableFlash})
    };
    a.connect = function (a, b) {
        ...
        var e = this, f = !1, g = this.transport.createConnection(this.name, this.priority, this.options.key, this.options), j = function () {
            g.unbind("initialized", j);
            g.connect()
        }, k = function () {
            new Pusher.Handshake(g, function (a) {
                f = !0;
                l();
                b(null, a)
            })
...
(function () {
    function b(a, b, c, f) {
        ...
        this.options = {encrypted: Boolean(f.encrypted), hostUnencrypted: f.hostUnencrypted, hostEncrypted: f.hostEncrypted}
    }
    ...
    a.initialize = function () {
        this.timeline.info(this.buildTimelineMessage({transport: this.name + (this.options.encrypted ? "s" : "")}));
    ...
    a.connect = function () {
        ...
        this.timeline.debug(this.buildTimelineMessage({method: "connect", url: a}));
        ...
        this.bindListeners();
        Pusher.debug("Connecting", {transport: this.name, url: a});
    ...
    a.send = function (a) {
        this.timeline.debug(this.buildTimelineMessage({method: "send", data: a}));
        if (this.state === "open") {
            var b =
                this;
            setTimeout(function () {
    ...
    a.onError = function (a) {
        ...
        this.timeline.error(this.buildTimelineMessage({error: c(a)}))
    };
    ...
    a.onMessage = function (a) {
        this.timeline.debug(this.buildTimelineMessage({message: a.data}));
        this.emit("message",
            a)
    };
...
(function () {
    ...
    c.initialize = function () {
        var a = this;
        this.timeline.info(this.buildTimelineMessage({transport: this.name +
...
(function () {
    ...
    c.createSocket = function (a) {
        return new SockJS(a, null, {js_path: Pusher.Dependencies.getPath("sockjs", {encrypted: this.options.encrypted}), ignore_null_origin: this.options.ignoreNullOrigin})
    };
    ...
    c.onOpen = function () {
        this.socket.send(JSON.stringify({path: Pusher.AbstractTransport.prototype.getPath.call(this) +
...
(function () {
    function b(a, b, c) {
        this.manager = a;
        this.transport = b;
        this.minPingDelay = c.minPingDelay || 1E4;
        this.maxPingDelay = c.maxPingDelay || Pusher.activity_timeout;
        this.pingDelay = null
    }

    var c = b.prototype;
    c.createConnection = function (a, b, c, e) {
        var f = this.transport.createConnection(a, b, c, e), g = this, j = null, k = null, m = function () {
            f.unbind("open", m);
            j = Pusher.Util.now();
            g.pingDelay && (k = setInterval(function () {
                k && f.requestPing()
            }, g.pingDelay));
            f.bind("closed", i)
        }, i = function (a) {
            ...
            k && (clearInterval(k), k =
                null);
            if (!a.wasClean && j && (a = Pusher.Util.now() - j, a < 2 * g.maxPingDelay))g.manager.reportDeath(), g.pingDelay = Math.max(a / 2, g.minPingDelay)
        };
        f.bind("open", m);
        return f
    };
...
(function () {
    ...
    function c(a, b) {
        ...
        var e = d(a[0], b), h = c(a.slice(1), e[1]);
    ...
    var h = {ws: Pusher.WSTransport, flash: Pusher.FlashTransport, sockjs: Pusher.SockJSTransport}, e = {def: function (a, b, c) {
        if (a[b] !== void 0)throw"Redefining symbol " + b;
        a[b] = c;
        return[void 0, a]
    }, def_transport: function (a, b, c, d, e, i) {
        var l =
            h[c];
        if (!l)throw new Pusher.Errors.UnsupportedTransport(c);
        c = Pusher.Util.extend({}, {key: a.key, encrypted: a.encrypted, timeline: a.timeline, disableFlash: a.disableFlash, ignoreNullOrigin: a.ignoreNullOrigin}, e);
        i && (l = i.getAssistant(l));
        d = new Pusher.TransportStrategy(b, d, l, c);
        i = a.def(a, b, d)[1];
        i.transports = a.transports || {};
        i.transports[b] = d;
        return[void 0, i]
        ...
        return new Pusher.CachedStrategy(c, a.transports, {ttl: b, timeline: a.timeline})
...
(function () {
    Protocol = {decodeMessage: function (b) {
        try {
        ...
        if (b.event === "pusher:connection_established")return{action: "connected", id: b.data.socket_id}; else if (b.event === "pusher:error")return{action: this.getCloseAction(b.data),
            error: this.getCloseError(b.data)}; else throw"Invalid handshake";
        ...
        return b.code !== 1E3 && b.code !== 1001 ? {type: "PusherError", data: {code: b.code, message: b.reason || b.message}} : null
    }};
...
(function () {
    ...
    c.bindListeners = function () {
        var a = this, b = function () {
            a.transport.unbind("message", c);
            a.transport.unbind("closed", e)
        }, c = function (c) {
            b();
            try {
                var e = Pusher.Protocol.processHandshake(c);
                e.action === "connected" ? a.finish("connected", {connection: new Pusher.Connection(e.id, a.transport)}) : (a.finish(e.action, {error: e.error}), a.transport.close())
            } catch (h) {
                a.finish("error", {error: h}), a.transport.close()
            }
        }, e = function (c) {
            b();
            var e = Pusher.Protocol.getCloseAction(c) || "backoff", c = Pusher.Protocol.getCloseError(c);
            a.finish(e, {error: c})
        };
        a.transport.bind("message", c);
        a.transport.bind("closed", e)
    };
...
(function () {
    ...
    c.connect = function () {
        var a = this;
        if (!a.connection && a.state !== "connecting")if (a.strategy.isSupported())if (Pusher.Network.isOnline() === !1)a.updateState("unavailable"); else {
            a.updateState("connecting");
            a.timelineSender =
                a.options.getTimelineSender(a.timeline, {encrypted: a.encrypted}, a);
            var b = function (c, e) {
    ...
    c.retryIn = function (a) {
        var b = this;
        this.retryTimer = new Pusher.Timer(a || 0, function () {
            b.disconnect();
            b.connect()
        })
    };
    ...
    c.updateState = function (a, b) {
        var c = this.state;
        this.state = a;
        c !== a && (Pusher.debug("State changed", c + " -> " + a), this.emit("state_change", {previous: c, current: a}), this.emit(a, b))
    };
...
(function () {
    ...
    Pusher.Channels.prototype = {add: function (b, a) {
        var d = this.find(b);
        d || (d = Pusher.Channel.factory(b, a), this.channels[b] = d);
        return d
    }, find: function (b) {
    ...
    Pusher.Channel = function (b, a) {
        ...
        this.bind("pusher_internal:subscription_succeeded", function (a) {
            d.onSubscriptionSucceeded(a)
        })
    };
    Pusher.Channel.prototype = {init: function () {
    }, disconnect: function () {
        this.subscribed = !1;
    ...
    }, authorize: function (b, a, d) {
        return d(!1, {})
    }, trigger: function (b, a) {
        return this.pusher.send_event(b, a, this.name)
    }};
    Pusher.Util.extend(Pusher.Channel.prototype, Pusher.EventsDispatcher.prototype);
    Pusher.Channel.PrivateChannel = {authorize: function (b, a, d) {
        var h = this;
        return(new Pusher.Channel.Authorizer(this, Pusher.channel_auth_transport, a)).authorize(b, function (a, b) {
            a || h.emit("pusher_internal:authorized", b);
            d(a, b)
        })
    }};
    Pusher.Channel.PresenceChannel = {init: function () {
        this.members = new b(this)
    }, onSubscriptionSucceeded: function () {
        this.subscribed = !0
    }};
    var b = function (b) {
        var a = this, d = null, h = function () {
            a._members_map = {};
            a.count = 0;
            d = a.me = null
        };
        h();
        var e = function (e) {
            a._members_map = e.presence.hash;
            a.count =
                e.presence.count;
            a.me = a.get(d.user_id);
            b.emit("pusher:subscription_succeeded", a)
        };
        b.bind("pusher_internal:authorized", function (a) {
            d = JSON.parse(a.channel_data);
            b.bind("pusher_internal:subscription_succeeded", e)
        });
        b.bind("pusher_internal:member_added", function (d) {
            a.get(d.user_id) === null && a.count++;
            a._members_map[d.user_id] = d.user_info;
            b.emit("pusher:member_added", a.get(d.user_id))
        });
        b.bind("pusher_internal:member_removed", function (d) {
            var e = a.get(d.user_id);
            e && (delete a._members_map[d.user_id], a.count--,
                b.emit("pusher:member_removed", e))
        });
        b.bind("pusher_internal:disconnected", function () {
            h();
            b.unbind("pusher_internal:subscription_succeeded", e)
        })
    };
    ...
    Pusher.Channel.factory = function (b, a) {
        var d = new Pusher.Channel(b, a);
        b.indexOf("private-") === 0 ? Pusher.Util.extend(d, Pusher.Channel.PrivateChannel) : b.indexOf("presence-") === 0 && (Pusher.Util.extend(d,
            Pusher.Channel.PrivateChannel), Pusher.Util.extend(d, Pusher.Channel.PresenceChannel));
        d.init();
        return d
    }
}).call(this);
(function () {
    Pusher.Channel.Authorizer = function (b, c, a) {
        this.channel = b;
        this.type = c;
        this.authOptions = (a || {}).auth || {}
    };
    Pusher.Channel.Authorizer.prototype = {composeQuery: function (b) {
        var b = "&socket_id=" + encodeURIComponent(b) + "&channel_name=" + encodeURIComponent(this.channel.name), c;
        for (c in this.authOptions.params)b += "&" + encodeURIComponent(c) + "=" + encodeURIComponent(this.authOptions.params[c]);
        return b
    }, authorize: function (b, c) {
        return Pusher.authorizers[this.type].call(this, b, c)
    }};
    Pusher.auth_callbacks = {};
    Pusher.authorizers = {ajax: function (b, c) {
        var a;
        ...
        for (var d in this.authOptions.headers)a.setRequestHeader(d, this.authOptions.headers[d]);
        a.onreadystatechange = function () {
            if (a.readyState == 4)if (a.status == 200) {
                var b, d = !1;
                try {
                    b = JSON.parse(a.responseText), d = !0
                } catch (f) {
