...
describe EventsController do
  describe 'routes' do
    ...
    it 'get index event' do
      expect(:get => 'admin/events').to route_to(:action => 'events', :controller => 'admin/users')
    end
    it 'get comments event' do
      expect(:get => "admin/users/#{@user.id}/comments").to route_to(:action => 'comments', :controller => 'admin/users', :user_id => "#{@user.id}")
    end
    it 'get likes event' do
      expect(:get => "admin/users/#{@user.id}/likes").to route_to(:action => 'likes', :controller => 'admin/users', :user_id => "#{@user.id}")
    end
    it 'get sign_out event' do
      expect(:get => "admin/users/#{@user.id}/sign_out").to route_to(:action => 'sign_out', :controller => 'admin/users', :user_id => "#{@user.id}")
    end
    it 'get sign_in event' do
      expect(:get => "admin/users/#{@user.id}/sign_in").to route_to(:action => 'sign_in', :controller => 'admin/users', :user_id => "#{@user.id}")
    end
    it 'get navigation event' do
      expect(:get => "admin/users/#{@user.id}/navigation").to route_to(:action => 'navigation', :controller => 'admin/users', :user_id => "#{@user.id}")
    end
  end
